...
export default function AdminHome() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const [profileResponse, statsResponse, questionsResponse] =
        await Promise.all([
          axios.get(`${API_BASE_URL}/api/profile`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          // CHANGED: Updated to use /api/dashboard/stats instead of /api/admin/dashboard/stats
      ...
      <div className="bg-gradient-to-r from-cyan-500 to-cyan-600 rounded-xl p-6 text-white">
        <div className="flex items-center gap-4">
          <Avatar className="h-16 w-16 ring-4 ring-white/20">
            <AvatarFallback
              className={cn("text-2xl font-bold", avatarColorClass)}
            >
              {avatarInitials}
            </AvatarFallback>
          </Avatar>
          <div>
            ...
            <p className="text-cyan-100">
              Ready to manage your community and create amazing content?
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-gray-600 text-sm leading-relaxed">
                Welcome to the Uttar Admin Panel - your centralized hub for
                managing the community platform. Here you can oversee questions,
                moderate content, manage users, and track platform analytics.
              </p>
            </div>
            <div>
              <h3 className="font-semibold text-gray-900 mb-2">Your Role</h3>
              <p className="text-gray-600 text-sm leading-relaxed">
                As an administrator, you have the power to shape the community
                experience. Monitor engagement, ensure quality content, and help
                create a thriving knowledge-sharing environment.
              </p>
            </div>
          </div>
        </CardContent>
